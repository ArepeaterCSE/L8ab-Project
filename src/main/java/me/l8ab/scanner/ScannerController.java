    ...
    public Map<String, Object> scanUrl(@RequestBody Map<String, String> payload) {
        ...
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder().uri(URI.create(targetUrl))
                    .header("User-Agent", "L8ab-Security-Scanner/1.0 (Educational)").build();
            ...
            result.put("status", "error");
            result.put("message", "Could not connect: " + e.getMessage());
        }
        return result;
    }
    private void checkHeader(Map<String, List<String>> headers, List<String> vulns, String key, String msg) {
        if (headers.keySet().stream().noneMatch(k -> k.equalsIgnoreCase(key))) vulns.add(msg);
    }
